...
zh_kgroups, _ = wrapper.kernel_kgroups(k, Y, G)
...
#ax.fill_between(X_plot[:,0], np.exp(log_dens1), facecolor="none", 
#                hatch=hatch, edgecolor='k', lw=0)
ax.fill_between(X_plot[:,0], np.exp(log_dens1), color="k", alpha=.1)
ax.plot(X_plot[:,0], np.exp(log_dens1), color='k', label='truth', lw=lw)
#ax.fill_between(X_plot[:,0], np.exp(log_dens2), facecolor='none',
#                hatch=hatch, edgecolor='k', lw=0)
ax.fill_between(X_plot[:,0], np.exp(log_dens2), color='k', alpha=.1)
ax.plot(X_plot[:,0], np.exp(log_dens2), color='k', lw=lw)

xs = np.linspace(low,high,num_points)
ax.plot(xs, scipy.stats.norm.pdf(xs, x1_mu_kmeans, np.sqrt(x1_var_kmeans)),
        label="%s"%(methods[0]), color=colors[0], lw=lw,
        marker='p', markevery=mk, fillstyle=fs, markersize=ms)
ax.plot(xs, scipy.stats.norm.pdf(xs, x2_mu_kmeans, np.sqrt(x2_var_kmeans)),
        color=colors[0], lw=lw, marker='p', markevery=mk, fillstyle=fs,
        markersize=ms)
ax.plot(xs, scipy.stats.norm.pdf(xs, x1_mu_gmm, np.sqrt(x1_var_gmm)),
        label="%s"%(methods[1]), color=colors[1], lw=lw,
        marker='o', markevery=mk, fillstyle=fs, markersize=ms)
ax.plot(xs, scipy.stats.norm.pdf(xs, x2_mu_gmm, np.sqrt(x2_var_gmm)),
        color=colors[1], lw=lw, marker='o', markevery=mk, fillstyle=fs,
        markersize=ms)
...
ax.plot(X_plot[:,0], np.exp(log_dens1), 
        label="%s"%(methods[2]), color=colors[2], lw=lw,
        marker='s', markevery=mk, fillstyle=fs, markersize=ms)
ax.plot(X_plot[:,0], np.exp(log_dens2), color=colors[2], lw=lw,
        marker='s', markevery=mk, fillstyle=fs, markersize=ms)
