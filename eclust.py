...
def normalized_rbf(X, k=10):
    """Compute a normalized RBF kernel, i.e.
    
    $$ G_{ij} =  \exp\{ -\| x_i - x_j\|^2/(\sigma_i \sigma_j) \} $$

    where \sigma_i is the distance from x_i to it's k-nearest neighbor
    
    """
    G = euclidean_distances(X)
    sigmas = np.sort(G, axis=1)[:,k] # pick from k-th nearest neighbor
    for i in range(G.shape[0]):
        for j in range(i, G.shape[0]):
            ...
            if sigma_j == 0:
                sigma_j = 1
            G[i,j] = np.exp(-G[i,j]**2/sigma_i/sigma_j)
            G[j,i] = G[i,j]
    return G
...
def kernel_kgroups(k, G, Z0, W, max_iter=100, tol=1e-4, verbose=False,
    ...
    # I think there should be a square root, but before it was as above
    #W2 = np.sqrt(W)
    #Gtilde = W2.dot(G.dot(W2)) # absorb weights into a new matrix
    ...
    if verbose:
        if count >= max_iter:
            print("\tKernel k-groups didn't converge in %i iterations." % count)
        else:
