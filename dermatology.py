...
# trying something else, normalized RBF
G = eclust.normalized_rbf(data, k=10)

r = []
r.append(wrapper.kmeans(6, data, run_times=10))
r.append(wrapper.gmm(6, data, run_times=10))
r.append(wrapper.spectral_clustering(6, data, G, run_times=10)[0])
Zh = eclust.ztoZ(r[-1])

r.append(wrapper.kernel_kmeans(6, data, G, run_times=10, Z0=Zh)[0])

r.append(wrapper.kernel_kgroups(6,data,G,run_times=10, Z0=Zh)[0])
