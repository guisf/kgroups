...
def topeigen(k, G, W, run_times=1, init='k-means++'):
    ...
    for i in range(n):
        K[i,i] = 0
    D = np.diag(K.sum(axis=1)) + (0.0001)*np.eye(K.shape[0])
    
    # this solves K v = \lambda D v, i.e. we solve the eigen problem
    # for D^{-1} K
    eigvals, Y = eigh(K, D, eigvals=(n-k,n-1))
    Yt = np.array([D.dot(Y[:,i]) for i in range(Y.shape[1])]).T
    #Yt = normalize(Yt, axis=1)
...
def topeigen2(k, G, W, run_times=1, init='k-means++'):
    ...
    for i in range(n):
        K[i,i] = 0
    D = np.diag(K.sum(axis=1)) + (0.00001)*np.eye(K.shape[0])
...
def spectralNg(k, G, W, run_times=1, init='k-means++', largest=True):
    ...
    if largest:
        ...
        eigvals, Y = eigh(K, D, eigvals=(0,k-1))
...
def spectral2(k, G, run_times=1, init='k-means++', largest=True):
    n, _ = G.shape
    if largest:
        eigvals, Y = eigh(G, eigvals=(n-k,n-1))
    else:
        eigvals, Y = eigh(G, eigvals=(0,k-1))
    Yt = np.array([Y[:,i] for i in range(Y.shape[1])]).T
    #Yt = normalize(Yt, axis=1)
    km = KMeans(k, init=init, n_init=run_times)
    labels = km.fit_predict(Yt)
    return labels
