...
def initialize(method, k, G, X, W, Z0=None):
    if Z0 is not None:
        return Z0
    if method == "spectral":
...
def kernel_kmeans(k, X, G, W=None, run_times=5, ini="k-means++", Z0=None):
    if type(W) == type(None):
        W = np.eye(len(X))
    best_score = -np.inf
    for _ in range(run_times):
        Z0 = initialize(ini, k, G, X, W, Z0)
...
def kernel_kgroups(k, X, G, W=None, run_times=5, ini="k-means++", Z0=None):
    if type(W) == type(None):
        W = np.eye(len(X))
    best_score = -np.inf
    for _ in range(run_times):
        Z0 = initialize(ini, k, G, X, W, Z0)
        zh = eclust.kernel_kgroups(k, G, Z0, W, max_iter=500)
        Zh = eclust.ztoZ(zh)
        score = eclust.objective(Zh, G, W)
        if score > best_score:
            best_score = score
...
def spectral(k, X, G, W=None, run_times=5):
    ...
    for _ in range(run_times):
        ...
        #zh = init.topeigen2(k, G, W, run_times=run_times)
        #zh = init.spectralNg(k, G, W, run_times=run_times)
        Zh = eclust.ztoZ(zh)
        score = eclust.objective(Zh, G, W)
        if score > best_score:
            best_score = score
...
def spectralNg(k, X, G, W=None, run_times=5, ini=None):
    if type(W) == type(None):
        W = np.eye(len(X))
    best_score = -np.inf
    for _ in range(run_times):
...
def kmeans(k, X, run_times=5, ini='k-means++'):
    km = KMeans(k, n_init=run_times, init=ini)
    km.fit(X)
    zh = km.labels_
    mu = km.cluster_centers_
    return zh

def gmm(k, X, run_times=5, ini='kmeans'):
    gm = GMM(k, n_init=run_times, init_params=ini)
    #gm = GMM(k)
    gm.fit(X)
    zh = gm.predict(X)
    mu = gm.means_
...
def spectral_clustering(k, X, G, W=None, run_times=5, ini=None):
    if type(W) == type(None):
        W = np.eye(len(X))
    W2 = np.sqrt(W)
    ...
    Zh = eclust.ztoZ(zh)
    score = eclust.objective(Zh, Gtilde, W)
    return zh, score
