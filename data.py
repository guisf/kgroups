...
def circles(radius, epsilon, numpoints):
    ...
    X = []
    z = []
    for r, e, n in zip(radius, epsilon, numpoints):
    ...
    X = np.concatenate(X)
    z = np.concatenate(z)
    idx = np.random.permutation(sum(numpoints))
    return X[idx], z[idx]
...
def plot(X, z, fname='plot.pdf'):
    ...
    #colors = iter(['#e41a1c', '#377eb8', '#4daf4a'])
    colors = iter(['#f16913', '#4292c6'])
    
    for k in z_unique:
        idx = np.where(z==k)
        x = X_new[idx][:,0]
        y = X_new[idx][:,1]
        ax.plot(x, y, 'bo', markersize=6, alpha=.8, color=next(colors))
...
def plot2(X, z, fname='plot.pdf'):
    ...
    ax = fig.add_subplot(111)
    colors = cm.gist_rainbow(np.linspace(0, 1, len(z_unique)))
    for k in z_unique:
        idx = np.where(z==k)
        x = X_new[idx][:,0]
        y = X_new[idx][:,1]
        ax.plot(x, y, 'bo', markersize=4, color=colors[k], alpha=.5)
    ax.set_xticks([])
    ax.set_yticks([])
    #plt.axes().set_aspect('equal', 'datalim')
    fig.tight_layout()
...
if __name__ == '__main__':
    ...
    plot(X, z, fname='./gauss_means_scatter.pdf')
    ...
    X, z = multivariate_normal([m1, m2], [s1, s2], [n1, n2])
    plot(X, z, fname='./gauss_cov_scatter.pdf')
    ...
    s2 = np.eye(2)
    X, z = multivariate_normal([m1, m2], [s1, s2], [n1, n2])
    plot(X, z, fname='./gauss_diff_metrics.pdf')
    X, z = multivariate_lognormal([m1, m2], [s1, s2], [n1, n2])
    plot(X, z, fname='./loggauss_diff_metrics.pdf')
    ...
    s2 = 0.5*np.eye(2)
    X, z = multivariate_normal([m1, m2], [s1, s2], [n1, n2])
    plot(X, z, fname='./unbalanced_scatter.pdf')
